...
class TGrabberCore
{
    ...
    function getContent($url)
    {
        ...
        // CURL
        {
            ...
            if ($this->config->get('getContentWriteLogsOn'))
                file_put_contents($this->tmpDir . "curl_getinfo_" . md5($url) . ".txt", curl_getinfo($ch, CURLINFO_HEADER_OUT));
        ...
        if ($this->config->get('getContentWriteLogsOn'))
            file_put_contents($this->tmpDir . md5($url) . ".html", $out);
        return $out;
    }
    ...
    function copyUrlFile($url, $file)
    {
        ...
        {
            if ($this->config->get('curlinfoHeaderOutOn')) $this->_echo('<br /><b>saveFileUrlMethod</b><i>1 (curl) </i> <br />');
    ...
    private function _import()
    {
        ...
        elseif ($this->feed['type'] == 'vk') // vk
        {
            ...
            # <div class="post_date"><a  class="post_link"  href="/wall-38574922_803"  onclick="return showWiki({w: 'wall-38574922_803'}, false, event);" ><span class="rel_date">30 окт 2017</span></a></div>
    ...
    function getMDNameFile($file)
    {
        static $ext = false;
        if (!$ext)
            $ext = preg_replace('|\?.*?|is', '', pathinfo($file, PATHINFO_EXTENSION));
        #$this->_echo('<br />file:'.$file.', file_ext: '. $ext . 'or:'. pathinfo($file, PATHINFO_EXTENSION).'<br/> ');
        #if (!$ext) $ext = 'jpg';
        $file = $this->rootPath . $this->config->get('imgPath') . $this->imageDir . md5(microtime() + mt_rand(1, 100)) . ".$ext";
        if (is_file($file))
            return $this->getMDNameFile($file);
        return $file;
    }
    ...
    function imageParser($matches)
    {
        ...
        if ($this->feed['params']['image_save']) // saving images on the server
        {
            ...
            if ($this->copyUrlFile($matches[3], $imageFile))
            {
                ...
                if ($this->introPicOn and ($this->feed['params']['intro_pic_on'] or $this->feed['params']['image_intro_on']))
                    $this->introPicOn($imageFile, 1, "{$matches[1]} {$matches[4]}");
                ...
                $this->_echo(' - <a href="//' . $_SERVER["SERVER_NAME"] . $matches[3] . '" style="color:green; font-weight: bold">OK</a>');
                // resizing
        ...
        else // without saving
        {
            if ($this->feed['params']['image_resize']) // resizing
            {
                if ($this->introPicOn and $this->feed['params']['intro_pic_on'])
